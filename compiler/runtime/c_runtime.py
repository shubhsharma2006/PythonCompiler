...
class CRuntimeSupport:
    ...
    @staticmethod
    def truthy_call(value_name: str, value_type: ValueType) -> str:
        if value_type == ValueType.FLOAT:
            return f"py_truthy_float({value_name})"
        if value_type == ValueType.STRING:
            return f"py_truthy_str({value_name})"
        if value_type == ValueType.LIST:
            return f"py_truthy_list({value_name})"
        if value_type == ValueType.TUPLE:
            return f"py_truthy_tuple({value_name})"
        return f"py_truthy_int({value_name})"
